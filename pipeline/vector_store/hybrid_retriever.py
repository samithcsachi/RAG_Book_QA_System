...
class HybridRetriever(VectorStoreBase):
    ...
    def search(self, query_embed, query_text, k=5, method=None):
        ...
        # Simple hybrid: combine and sort by average rank/score (tune as
        # desired)
        ...
        for text in all_texts:
            ...
            joint_score = self.alpha * \
                (k - f_rank) + (1 - self.alpha) * (k - b_rank)
            # Prefer faiss meta but fallback to bm25
            meta = faiss_hits[faiss_ids[text]
                              ]["meta"] if text in faiss_ids else bm25_hits[bm25_ids[text]]["meta"]
